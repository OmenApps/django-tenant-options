...
class Command(BaseCommand):
    ...
    def syncoptions(self):
        ...
        if not model_subclasses:
            self.stdout.write(self.style.WARNING("No default options found in the project."))
            return

        for ModelClass in model_subclasses:
    ...
    def update_options(self, ModelClass):
    ...
    def print_updated_options(self, ModelClass, updated_options):
    ...
    def print_imported_or_verified_options(self, updated_options):
        ...
        if imported_count > 0:
            self.stdout.write(self.style.WARNING("  Imported or Verified Options:"))
            ...
            self.stdout.write(self.style.WARNING("  No options imported or verified"))
    ...
    def print_newly_deleted_options(self, updated_options):
        ...
        if deleted_count > 0:
            self.stdout.write(self.style.WARNING("  Newly Deleted Options:"))
            for name, options_dict in updated_options.items():
                if options_dict.get("deleted") is not None:
                    self.stdout.write(f"    - '{name}'")
            self.stdout.write(f"    {deleted_count} Newly Deleted Options")
        else:
            self.stdout.write(self.style.WARNING("  No Newly Deleted Options"))

    def print_all_pre_existing_deleted_options(self, ModelClass, updated_options):
        ...
        if all_pre_existing_deleted.exists():
            self.stdout.write(self.style.WARNING("  All Pre-existing Deleted Options:"))
            ...
            self.stdout.write(self.style.WARNING("  No Pre-existing Deleted Options"))

    def print_all_active_custom_options(self, ModelClass):
        """Prints all custom options."""
        custom_options = ModelClass.objects.custom_options().active()
        if custom_options.exists():
            self.stdout.write(self.style.WARNING("  All Custom Options:"))
            for option in custom_options:
                self.stdout.write(f"    - '{option.name}', Tenant: {option.tenant}")
            self.stdout.write(f"    {custom_options.count()} Custom Options")
        else:
            self.stdout.write(self.style.WARNING("  No Custom Options"))
